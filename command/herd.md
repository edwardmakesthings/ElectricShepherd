...
4. Separately, list existing closets with `es-status: provisional` in scope — these were
   already synthesized but never validated/promoted, so they are currently hidden from
   default retrieval. This is a different backlog than item 1 (nothing new to synthesize,
   just nothing yet run through dream-auditor).
