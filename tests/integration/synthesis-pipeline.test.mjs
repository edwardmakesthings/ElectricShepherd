...
/**
 * End-to-end integration coverage for the mem_raw -> mem_synth -> retrieval
 ...
 * envelopes are well-formed): we seed real mem_raw drawers, run the real
 * consolidation with writes enabled, and then assert that the resulting
 * synthesis node is BOTH (a) discoverable by scope via
 * `findScopedSynthesisNodes` — a hard, similarity-independent guarantee — and
 * (b) surfaced by the retrieval-expansion adapter for the room.
 *
 * We provide mapper summaries keyed to the seeded drawer ids so the synthesis
 * node references genuine source drawers, while keeping the test independent of
...
test("raw drawers consolidate into a synthesis node that is then discoverable", { skip: !runIntegration }, async () => {
  // 1) Seed real mem_raw drawers in the isolated room and capture their ids.
  ...
  const seededB = await room.addDrawer(
    "ElectricShepherd consolidation: the dedup gate stays configurable via " +
      "ESHEPHERD_MEMRAW_DEDUP_ENABLED. Subsystems: adapter/synthesis-consolidation.ts and " +
      "scripts/capture-memraw.sh. Open item: end-to-end retrieval verification."
  );
  ...
  //    to the seeded drawer ids so the synthesis node references real sources.
  const consolidation = await runSynthesisConsolidation(room.client, {
    ...
    mapperSummaries: [
      ...
      },
      {
        ...
        confidence: "medium",
        durableFacts: ["The dedup gate is configurable via ESHEPHERD_MEMRAW_DEDUP_ENABLED"],
  ...
  assert.equal(typeof consolidation.createdNodeId, "string", "a synthesis node must be created");
  // Ensure teardown removes the synth node as well.
  room.track(consolidation.createdNodeId);

  // 3) Hard guarantee: the synth node is discoverable by scope.
  const scoped = await room.client.findScopedSynthesisNodes({
  ...
  assert.ok(
    ...
    `findScopedSynthesisNodes did not return the new node. got: ${JSON.stringify(scopedIds)}`
  );
