...
test("never runs when disabled", () => {
  const d = decideAutoConsolidation(base({ enabled: false, trigger: "compacted", messagesSinceRun: 999 }));
  assert.equal(d.shouldRun, false);
  assert.equal(d.reason, "disabled");
});

test("never runs while a previous run is in flight", () => {
  const d = decideAutoConsolidation(base({ inFlight: true, trigger: "compacted", messagesSinceRun: 999 }));
  assert.equal(d.shouldRun, false);
  assert.equal(d.reason, "in-flight");
});

test("respects the cooldown window", () => {
  const d = decideAutoConsolidation(
...
test("volume trigger waits for the message threshold", () => {
  const below = decideAutoConsolidation(base({ trigger: "volume", messagesSinceRun: THRESHOLD - 1 }));
  assert.equal(below.shouldRun, false);
  assert.equal(below.reason, "below-threshold");

  const at = decideAutoConsolidation(base({ trigger: "volume", messagesSinceRun: THRESHOLD }));
...
test("idle-timer requires at least one new turn since the last run", () => {
  const noActivity = decideAutoConsolidation(base({ trigger: "idle-timer", messagesSinceRun: 0 }));
  assert.equal(noActivity.shouldRun, false);
  assert.equal(noActivity.reason, "no-activity");

  const withActivity = decideAutoConsolidation(base({ trigger: "idle-timer", messagesSinceRun: 1 }));
