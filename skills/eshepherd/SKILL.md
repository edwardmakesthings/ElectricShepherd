...
| `es-source-type` | What KIND of material a drawer holds: `transcript` \| `note` \| `doc` \| `synthesis` \| `skill`. ORTHOGONAL to `es-status` (kind vs validated) — never conflate them. `note` is first-party prose authored during work (diary, research write-ups) with no `synthesized-from` lineage: above a transcript, below a synthesis. Stamped at write time; missing = "unknown authority", never a default type. Backfill infers `transcript` from transcript-like rooms, `synthesis` from outgoing `synthesized-from` edges, then `note` from note-like rooms with no such lineage, and leaves everything else unstamped. | capture pipeline / consolidation / `palace_stamp_source_type` backfill |
