/**
 * Capability routing tiering — moved verbatim from
 ...
 * unchanged; the shape they consume (WorkedExampleShape) stays owned by
 * procedural/worked-examples.ts.
 */

import type { WorkedExampleShape } from "../procedural/worked-examples.ts";

/** Routing tier vocabulary for capability memory (learned routing). */
export const CAPABILITY_TIERS = ["local", "cloud", "deep"] as const;

export type CapabilityTier = (typeof CAPABILITY_TIERS)[number];

/** Closed outcome vocabulary for capability tuples (matches es-outcome). */
...
/**
 * Map a task tool part status to a capability outcome.
...
/**
 * Canonical shape summary string for a capability bucket. Deterministic
