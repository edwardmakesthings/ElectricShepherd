...
export function usage(): string {
  return [
    ...
    "",
    "Triage flags (pass 1 of two-pass consolidation):",
    "  --triage-only                    (run ONLY triage over the worklist, then stop)",
    "  --triage-model <provider/model>  (cheap large-context model; unset inherits the session model)",
    "  --triage-agent <name>            (default: drawer-triage)",
    "  --triage-batch-size <n>          (drawers per triage call; default: 10)",
    "  --triage-min-score <n>           (min spans to stay in the source room; default: 1)",
    "  --triage-rejected-room <room>    (default: <room>-triage-rejected)",
    "  --triage-timeout-ms <n>          (per triage call; default: 600000)",
    "  --keep-triage-sessions           (keep each triage pass as a readable harness session)",
    "",
