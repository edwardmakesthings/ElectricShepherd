...
import type { SynthesisConsolidationResult } from "../../adapter/synthesis-consolidation.ts";
import type { ValidationMergeReviewResult } from "../../adapter/validation-merge-review.ts";
...
export async function fetchPendingReminderLines(args: {
  ...
  try {
    ...
    const matches = matchRemindersForScope(
      reminders.map((reminder): import("../../adapter/prospective.ts").ReminderFact => ({
        ...
        status: (String(reminder.status || "active").trim().toLowerCase() || "active") as import("../../adapter/prospective.ts").ReminderStatus,
