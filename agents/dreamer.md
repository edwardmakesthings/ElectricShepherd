...
- Synthesis contract: when producing mem-synth output, write synthesis nodes with `create_synthesis_node` (and `apply_merge` when needed). Do not substitute `add_drawer` for synthesis nodes.
...
- Use `kg_add` for factual entity links; use `create_synthesis_node` for synthesis DAG structure.
...
4) Reduce mapper summaries into consolidated mem-synth memory by calling `create_synthesis_node` and capture created node IDs.
5) Run drift audit against scoped mem-core renders (`eshepherd/memory/**/memory.md`) and write one dream-log entry.
