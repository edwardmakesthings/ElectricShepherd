...
- Source-type vocabulary is CLOSED: `transcript`, `note`, `doc`, `synthesis`, `skill`. `note` is first-party prose authored during work (diary, agent research write-ups) with NO `synthesized-from` lineage — it outranks a transcript but is not a synthesis. Lineage always wins: a diary drawer that really was derived from sources is `synthesis`, not `note`. An unstamped drawer is `unknown`; never guess a type to fill the gap.
- Graphify output is DERIVED data — never file `graph.json`, `GRAPH_REPORT.md`, or node/edge listings as drawers. They are stale after the next refactor and regenerable in seconds, while drawers are append-only and cannot be retracted. File the decision the graph led you to, never the graph.
